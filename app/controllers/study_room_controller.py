...
class StudyRoomController:
    ...
    async def update_participant_permission(self, current_user_id: str, study_room_id: str, participant_id: str, permission: str):
        await self.study_room_service.update_participant_permission(current_user_id, study_room_id, participant_id, permission)
        return create_response(RESPONSE_STATUS_SUCCESS, "Participant's permission updated successfully")
    
    async def search_invitation_by_room(self, current_user_id: str, study_room_id:str, query: str):
        users = await self.study_room_service.search_invitation_by_room(current_user_id, study_room_id, query)
        return create_response(RESPONSE_STATUS_SUCCESS, "User's with study room invitations fetched successfully ", {"users": users})
